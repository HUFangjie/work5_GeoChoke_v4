...
class AionDefense(DefenseStrategy):
    """Single-process faithful simulation of Aion single-mask secure aggregation."""
    ...
    def __init__(self, cfg: Any, profiles: dict[str, dict[str, Any]], device: str = "cpu", output_dir: str | None = None) -> None:
        ...
        self.total_clients_seen: set[int] = set()
    ...
    def prepare_client_upload(self, client_id: int, update_vector: Any, round_id: int, profile_id: str, num_samples: int, metadata: dict[str, Any] | None = None):
        ...
        self.total_clients_seen.add(int(client_id))
        ...
        mask = self.protocol.client_mask(int(client_id), round_id, update.size)
        masked = update + alpha * mask
        if self.cfg.enable_dmc_dmr:
            masked = np.round(masked, int(self.cfg.decimal_places) + extra_digits)
        defense_metadata = {
            "aion_client_id": int(client_id),
            "aion_masked_update_norm": float(np.linalg.norm(masked)),
            "aion_mask_norm": float(np.linalg.norm(alpha * mask)),
    ...
    def filter_uploads_before_aggregation(self, uploads: list[Any], round_id: int):
        ...
        result = self.mgf.filter(list(uploads), round_id)
    ...
    def unmask_aggregate_update(self, aggregate_update: Any, uploads: list[Any], round_id: int, weights: list[float]):
        ...
        aggregate_mask, metrics = self.protocol.reconstruct_aggregated_mask(valid_ids, round_id, masked_aggregate.size, mean=True, mode=self.cfg.reconstruction_mode)
        ...
        self.mgf.note_global_update(unmasked_norm)
        metrics.update({
    ...
    def _check_uniform_aggregation(self, uploads: list[Any]) -> None:
        ...
        # Aion may run with cfg.aggregation="weighted_mean" only when uniform counts make it exactly mean.
        if len(counts) > 1:
